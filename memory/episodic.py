...
class EpisodicBuffer:
    ...
    def _init_schema(self):
        with self._lock:
            ...
            """)
            self._conn.execute("CREATE INDEX IF NOT EXISTS idx_eb_sid ON episodic_buffer(session_id)")
            self._conn.commit()

    async def add(self, summary: str, message_count: int = 0) -> None:
        def _do():
            with self._lock:
                self._conn.execute(
    ...
    async def get_recent(self, limit: int | None = None) -> list[dict]:
        ...
        def _do():
            with self._lock:
                ...
                return [
                    {"summary": r[0], "message_count": r[1], "created_at": r[2]}
    ...
    async def clear(self) -> None:
        def _do():
            with self._lock:
                self._conn.execute(
                    "DELETE FROM episodic_buffer WHERE session_id=?",
                    (self.session_id,),
                )
                self._conn.commit()
